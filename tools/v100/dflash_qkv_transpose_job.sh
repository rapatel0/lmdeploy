...
RESULTS=/results/$(date +%Y%m%d_%H%M%S)-dflash-qkv-transpose-${COMMIT:-unknown}
...
for arm in rowmajor transposed; do
 flag=0; [ "$arm" = transposed ] && flag=1
 echo "=== $arm TM_DFLASH_FP16_QKV_TRANSPOSE=$flag ==="
 TM_DFLASH_QK_NORM_WARP32=1 TM_DFLASH_FP16_QKV_TRANSPOSE="$flag" \
...
for arm in ('rowmajor','transposed'):
...
if sum('DFLASH_FP16_QKV_TRANSPOSE_ACTIVE' in (p/f'transposed.log').read_text(errors='replace') for _ in [0]) != 1:
 raise SystemExit('FAIL transpose route inactive')
print('DFLASH_QKV_TRANSPOSE_ANALYSIS_COMPLETE')
PY
echo '=== audited identity transposed ==='
TM_DFLASH_QK_NORM_WARP32=1 TM_DFLASH_FP16_QKV_TRANSPOSE=1 python3 /job/verify_dflash_audited.py \
...
echo DFLASH_QKV_TRANSPOSE_QUALIFICATION_COMPLETE
