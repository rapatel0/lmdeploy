...
run_arm() {
    local label=$1 k=$2 replay=$3
    echo "=== ${label}: K=${k} zero_replay=${replay} ==="
    local extra=()
    ...
    TM_MTP_LOCAL_TOP1=1 TM_MTP_TP_REDUCE=1 TM_MTP_EAGLE_ROTATION=1 \
        ...
        python3 /src/tools/v100/bench_decode.py --model "${MODEL_DIR}" --tp "${TP}" \
...
run_arm k4_replay 4 1 || exit $?
python3 - "${RESULTS}" <<'PY'
import json,os,sys
r=sys.argv[1]
vals={n:json.load(open(os.path.join(r,n+'.json')))['mean_decode_tok_s'] for n in ('k0','k4_control','k4_replay')}
