...
RESULTS=/results/$(date +%Y%m%d_%H%M%S)-dflash-viterbi-selector-${SRC_COMMIT:-unknown}
mkdir -p "$RESULTS"
...
for arm in control viterbi; do
    flag=0
    [ "$arm" = viterbi ] && flag=1
    TM_DFLASH_VITERBI_SELECTOR=$flag python3 /job/bench_decode.py "${common[@]}" \
        --json-out "$RESULTS/$arm.json" 2>&1 | tee "$RESULTS/$arm.log"
done
TM_DFLASH_VITERBI_SELECTOR=1 python3 /job/verify_dflash_audited.py \
...
for arm in ('control','viterbi'):
 data=json.loads((root/f'{arm}.json').read_text());matches=pattern.findall((root/f'{arm}.log').read_text(errors='replace'));assert matches
 commit,raw,steps=matches[-1]
 print(f'VERDICT {arm} decode={data["mean_decode_tok_s"]:.4f} commit={commit} raw={raw} steps={steps}')
PY
touch "$RESULTS/completed"
echo DFLASH_VITERBI_SELECTOR_COMPLETE
