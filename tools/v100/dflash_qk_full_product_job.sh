...
RESULTS=/results/$(date +%Y%m%d_%H%M%S)-dflash-qk-full-product-${COMMIT:-unknown}
...
for arm in half_product full_product; do
 flag=0; [ "$arm" = full_product ] && flag=1
 echo "=== $arm TM_DFLASH_QK_FULL_PRODUCT=$flag ==="
 TM_DFLASH_QK_NORM_WARP32=1 TM_DFLASH_QK_FULL_PRODUCT="$flag" \
...
for arm in ('half_product','full_product'):
...
print('DFLASH_QK_FULL_PRODUCT_ANALYSIS_COMPLETE')
PY
TM_DFLASH_QK_NORM_WARP32=1 TM_DFLASH_QK_FULL_PRODUCT=1 python3 /job/verify_dflash_audited.py \
...
echo DFLASH_QK_FULL_PRODUCT_QUALIFICATION_COMPLETE
