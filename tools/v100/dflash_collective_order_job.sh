#!/usr/bin/env bash
# Compare the old per-rank convolution order with SGLang's reduce-first order.
set -euo pipefail

SRC_COMMIT="$(sed -n 's/^commit=\(.\{12\}\).*/\1/p' /src/SOURCE_STAMP 2>/dev/null)"
[ -n "${SRC_COMMIT}" ] || SRC_COMMIT=unknown
RESULTS=/results/$(date +%Y%m%d_%H%M%S)-dflash-collective-order-${SRC_COMMIT}
mkdir -p "${RESULTS}"
...
for reduce_first in 0 1; do
    arm="reduce_first${reduce_first}"
    echo "=== COLLECTIVE_ORDER_ARM ${arm} ==="
    TM_DFLASH_REDUCE_BEFORE_CONV="${reduce_first}" \
...
rows = []
for result in sorted(root.glob("reduce_first*.json")):
...
echo DFLASH_COLLECTIVE_ORDER_COMPLETE
