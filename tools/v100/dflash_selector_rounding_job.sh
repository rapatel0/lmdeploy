#!/usr/bin/env bash
# Test SGLang-style FP16 predecessor*hidden narrowing in selector scoring.
set -euo pipefail

SRC_COMMIT="$(sed -n 's/^commit=\(.\{12\}\).*/\1/p' /src/SOURCE_STAMP 2>/dev/null)"
[ -n "${SRC_COMMIT}" ] || SRC_COMMIT=unknown
RESULTS=/results/$(date +%Y%m%d_%H%M%S)-dflash-selector-rounding-${SRC_COMMIT}
mkdir -p "${RESULTS}"
...
for fp16_product in 0 1; do
    arm="fp16_product${fp16_product}"
    echo "=== SELECTOR_ROUNDING_ARM ${arm} ==="
    TM_DFLASH_SELECTOR_FP16_PRODUCT="${fp16_product}" \
...
TM_DFLASH_SELECTOR_FP16_PRODUCT=1 \
    ...
    --expected-prompt-sha256 9ac441c0409e992b270fbe9cb47ca11bf00f66dc903dcd0fd32ad00b70007a01 \
    2>&1 | tee "${RESULTS}/fp16_product1_identity.log"
grep -q '^DFLASH_AUDITED_IDENTITY_PASS$' "${RESULTS}/fp16_product1_identity.log"
...
rows = []
for result in sorted(root.glob("fp16_product*.json")):
...
echo DFLASH_SELECTOR_ROUNDING_COMPLETE
