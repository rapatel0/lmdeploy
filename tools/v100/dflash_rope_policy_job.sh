#!/usr/bin/env bash
# Compare legacy target-global RoPE with each draft layer's checkpoint RoPE.
set -euo pipefail

SRC_COMMIT="$(sed -n 's/^commit=\(.\{12\}\).*/\1/p' /src/SOURCE_STAMP 2>/dev/null)"
[ -n "${SRC_COMMIT}" ] || SRC_COMMIT=unknown
RESULTS=/results/$(date +%Y%m%d_%H%M%S)-dflash-rope-policy-${SRC_COMMIT}
mkdir -p "${RESULTS}"
...
for per_layer in 0 1; do
    arm="per_layer_rope${per_layer}"
    echo "=== ROPE_POLICY_ARM ${arm} ==="
    TM_DFLASH_PER_LAYER_ROPE="${per_layer}" \
...
rows = []
for result in sorted(root.glob("per_layer_rope*.json")):
...
echo DFLASH_ROPE_POLICY_COMPLETE
