#!/usr/bin/env bash
# Compare legacy serial top-16 merge with CUB block reduction on V100.
set -euo pipefail

SRC_COMMIT="$(sed -n 's/^commit=\(.\{12\}\).*/\1/p' /src/SOURCE_STAMP 2>/dev/null)"
[ -n "${SRC_COMMIT}" ] || SRC_COMMIT=unknown
RESULTS=/results/$(date +%Y%m%d_%H%M%S)-dflash-cub-topk-${SRC_COMMIT}
mkdir -p "${RESULTS}"
...
for cub_topk in 0 1; do
    echo "=== TM_DFLASH_CUB_TOPK=${cub_topk} ==="
    TM_DFLASH_CUB_TOPK="${cub_topk}" python3 /job/bench_decode.py \
        ...
        --json-out "${RESULTS}/topk_${cub_topk}.json"
done

echo "=== local-top-k exact identity ==="
TM_DFLASH_CUB_TOPK=1 python3 /job/verify_dflash_audited.py \
