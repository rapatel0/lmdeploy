...
RESULTS=/results/$(date +%Y%m%d_%H%M%S)-dflash-tilelang-strict-parity-${SRC_COMMIT:-unknown}
mkdir -p "$RESULTS"
...
SG=${SGLANG_DFLASH_REPLAY_ROOT:-/results/20260901_021812-sglang-dflash-parity-5d8f38958e1e/trace/sglang}
mapfile -t REPLAY < <(
    python3 - "$SG" <<'PY'
...
for rank, (lm_root, sg_root) in enumerate(zip(lm_roots, sg_roots)):
    ...
    pairs = {
        ...
        'selector.candidate_ids': (load(lm_root, lm, 'selector.candidate_ids'), load(sg_root, sg, 'selector.candidate_ids')),
        'selector.selected_ids': (load(lm_root, lm, 'selector.selected_ids'), load(sg_root, sg, 'selector.selected_ids')),
    }
    exact_names = {
        'draft.q_attention',
        'draft.k_normalized',
