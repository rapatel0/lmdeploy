#!/usr/bin/env bash
# Qualify one contiguous TP4 target-verification CUDA graph on V100.
set -euo pipefail
SRC_COMMIT="$(sed -n 's/^commit=\(.\{12\}\).*/\1/p' /src/SOURCE_STAMP 2>/dev/null)"
RESULTS=/results/$(date +%Y%m%d_%H%M%S)-dflash-contiguous-target-graph-${SRC_COMMIT:-unknown}
mkdir -p "${RESULTS}"
...
for arm in baseline target_graph; do
    if [ "${arm}" = target_graph ]; then mode=1; trace=1; else mode=0; trace=0; fi
    echo "=== ${arm}: contiguous_target_graph=${mode} ==="
    TM_DFLASH_CONTIGUOUS_TARGET_GRAPH="${mode}" TM_DFLASH_GRAPH_TRACE="${trace}" \
        python3 /job/bench_decode.py "${common[@]}" --output-tokens 256 --trials 5 \
        --json-out "${RESULTS}/${arm}.json" 2>&1 | tee "${RESULTS}/${arm}.log"
done
captures="$(grep -c 'contiguous target graph captured' "${RESULTS}/target_graph.log" || true)"
replays="$(grep -c 'contiguous target graph replay' "${RESULTS}/target_graph.log" || true)"
[ "${captures}" -ge 4 ] && [ "${replays}" -ge 4 ] || {
    echo "FAIL: expected TP4 target graph capture/replay, got captures=${captures} replays=${replays}" >&2
    exit 3
}
...
export FT_NVTX=ON
for arm in baseline target_graph; do
    if [ "${arm}" = target_graph ]; then mode=1; else mode=0; fi
    TM_DFLASH_CONTIGUOUS_TARGET_GRAPH="${mode}" \
...
python3 /job/analyze_dflash_contiguous_target_graph.py "${RESULTS}" | tee "${RESULTS}/analysis.log"
