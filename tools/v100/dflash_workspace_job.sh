#!/usr/bin/env bash
# Compare dynamic versus phase-owned DFlash proposal workspaces on V100.
set -euo pipefail

SRC_COMMIT="$(sed -n 's/^commit=\(.\{12\}\).*/\1/p' /src/SOURCE_STAMP 2>/dev/null)"
[ -n "${SRC_COMMIT}" ] || SRC_COMMIT=unknown
RESULTS=/results/$(date +%Y%m%d_%H%M%S)-dflash-workspace-${SRC_COMMIT}
mkdir -p "${RESULTS}"
...
for workspace in 0 1; do
    echo "=== TM_DFLASH_PERSISTENT_WORKSPACE=${workspace} ==="
    TM_DFLASH_PERSISTENT_WORKSPACE="${workspace}" \
        TM_DFLASH_WORKSPACE_TRACE="${workspace}" \
        ...
        --json-out "${RESULTS}/workspace_${workspace}.json"
done

echo "=== persistent workspace exact identity ==="
...
echo DFLASH_WORKSPACE_COMPLETE
