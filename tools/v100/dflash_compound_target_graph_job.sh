#!/usr/bin/env bash
# Test whether the qualified target and draft graph capabilities compound materially.
set -euo pipefail
SRC_COMMIT="$(sed -n 's/^commit=\(.\{12\}\).*/\1/p' /src/SOURCE_STAMP 2>/dev/null)"
RESULTS=/results/$(date +%Y%m%d_%H%M%S)-dflash-compound-target-graph-${SRC_COMMIT:-unknown}
mkdir -p "${RESULTS}"
...
[ "$(grep -c 'contiguous target graph captured' "${RESULTS}/compound.log" || true)" -ge 4 ]
[ "$(grep -c 'draft graph captured' "${RESULTS}/compound.log" || true)" -ge 4 ]
...
echo DFLASH_COMPOUND_TARGET_GRAPH_COMPLETE
