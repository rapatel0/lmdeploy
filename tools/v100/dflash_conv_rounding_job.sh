#!/usr/bin/env bash
# Test SGLang-style FP16 operation boundaries in grouped convolution.
set -euo pipefail

SRC_COMMIT="$(sed -n 's/^commit=\(.\{12\}\).*/\1/p' /src/SOURCE_STAMP 2>/dev/null)"
[ -n "${SRC_COMMIT}" ] || SRC_COMMIT=unknown
RESULTS=/results/$(date +%Y%m%d_%H%M%S)-dflash-conv-rounding-${SRC_COMMIT}
mkdir -p "${RESULTS}"
...
for fp16_steps in 0 1; do
    arm="fp16_steps${fp16_steps}"
    echo "=== CONV_ROUNDING_ARM ${arm} ==="
    TM_DFLASH_CONV_FP16_STEPS="${fp16_steps}" \
...
TM_DFLASH_CONV_FP16_STEPS=1 \
    ...
    --expected-prompt-sha256 9ac441c0409e992b270fbe9cb47ca11bf00f66dc903dcd0fd32ad00b70007a01 \
    2>&1 | tee "${RESULTS}/fp16_steps1_identity.log"
grep -q '^DFLASH_AUDITED_IDENTITY_PASS$' "${RESULTS}/fp16_steps1_identity.log"
...
rows = []
for result in sorted(root.glob("fp16_steps*.json")):
...
echo DFLASH_CONV_ROUNDING_COMPLETE
