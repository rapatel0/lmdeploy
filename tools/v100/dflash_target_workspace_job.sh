#!/usr/bin/env bash
# Qualify stable batch-one Q=8 target-verification workspaces on TP4 V100.
set -euo pipefail
SRC_COMMIT="$(sed -n 's/^commit=\(.\{12\}\).*/\1/p' /src/SOURCE_STAMP 2>/dev/null)"
RESULTS=/results/$(date +%Y%m%d_%H%M%S)-dflash-target-workspace-${SRC_COMMIT:-unknown}
mkdir -p "${RESULTS}"
...
for arm in dynamic workspace; do
    if [ "${arm}" = workspace ]; then mode=1; trace=1; else mode=0; trace=0; fi
    echo "=== ${arm}: target_workspace=${mode} ==="
    TM_DFLASH_TARGET_WORKSPACE="${mode}" \
        TM_DFLASH_WORKSPACE_TRACE="${trace}" \
        python3 /job/bench_decode.py "${common[@]}" --output-tokens 256 --trials 5 \
        --json-out "${RESULTS}/${arm}.json" 2>&1 | tee "${RESULTS}/${arm}.log"
done

grep -q 'target GDN workspace' "${RESULTS}/workspace.log"
grep -q 'target FFN workspace' "${RESULTS}/workspace.log"
grep -q 'attention workspace' "${RESULTS}/workspace.log"
...
export FT_NVTX=ON
for arm in dynamic workspace; do
    if [ "${arm}" = workspace ]; then mode=1; else mode=0; fi
    TM_DFLASH_TARGET_WORKSPACE="${mode}" \
...
python3 /job/analyze_dflash_target_workspace.py "${RESULTS}" | tee "${RESULTS}/analysis.log"

TM_DFLASH_TARGET_WORKSPACE=1 \
...
echo DFLASH_TARGET_WORKSPACE_COMPLETE
