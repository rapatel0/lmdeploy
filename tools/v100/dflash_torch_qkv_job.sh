...
RESULTS=/results/$(date +%Y%m%d_%H%M%S)-dflash-torch-qkv-${COMMIT:-unknown}
...
for arm in packed torch_layout; do
 flag=0; [ "$arm" = torch_layout ] && flag=1
 echo "=== $arm TM_DFLASH_QKV_TORCH_LAYOUT=$flag ==="
 TM_DFLASH_QKV_TORCH_LAYOUT="$flag" python3 /job/bench_decode.py "${common[@]}" \
  --json-out "$RESULTS/$arm.json" 2>&1 | tee "$RESULTS/$arm.log"
done
python3 - "$RESULTS" <<'PY' | tee "$RESULTS/analysis.log"
import json,pathlib,re,sys
p=pathlib.Path(sys.argv[1])
for arm in ('packed','torch_layout'):
...
if 'DFLASH_QKV_TORCH_LAYOUT_ACTIVE' not in (p/'torch_layout.log').read_text(errors='replace'):
 raise SystemExit('FAIL torch QKV route inactive')
print('DFLASH_TORCH_QKV_ANALYSIS_COMPLETE')
PY
TM_DFLASH_QKV_TORCH_LAYOUT=1 python3 /job/verify_dflash_audited.py \
...
echo DFLASH_TORCH_QKV_QUALIFICATION_COMPLETE
