#!/usr/bin/env bash
# Run a script on V100 island 1, inside the prebuilt campaign image.
#
# The image already contains the toolchain, torch and the LMDeploy runtime
# requirements, so a job starts working immediately.
#
# Usage:
#   tools/v100/run_island1.sh <script.sh> [job-name] [extra-file ...]
#
...
#
# The island guard runs first. It aborts before any allocation if an island-2
# GPU is visible, if the visible count is not 4, or if island 1 is not idle.
set -euo pipefail
...
TM_DFLASH_LOCAL_TOPK="${TM_DFLASH_LOCAL_TOPK:-1}"
...
TM_MTP_AMBIGUITY_MARGIN="${TM_MTP_AMBIGUITY_MARGIN:-}"
TM_MTP_AMBIGUOUS_REPLAY="${TM_MTP_AMBIGUOUS_REPLAY:-0}"
...
SCRIPT="${1:?usage: run_island1.sh <script.sh> [job-name] [extra-file ...]}"
JOB="${2:-lmdeploy-v100-island1}"
...
python3 - "$JOB" "$NS" "$IMAGE" "$ISLAND1" "$ISLAND2" "$TP" "$NUM_DRAFT_TOKENS" "$MODEL_DIR" \
...
import json, sys
job, ns, image, island1, island2, tp, num_draft, model_dir, batch_copy_order, mtp_local_top1, dflash_local_topk, mtp_frozen_kv, mtp_eagle_rotation, mtp_force_reject, mtp_ambiguity_margin, mtp_ambiguous_replay, spec_trace, launch_blocking, cublas_workspace, nccl_debug, nccl_debug_subsys = sys.argv[1:22]

guard = r'''set -uo pipefail
for BAD in $ISLAND2_UUIDS; do
  if nvidia-smi --query-gpu=uuid --format=csv,noheader | grep -q "$BAD"; then
    echo "FATAL: island-2 GPU $BAD is visible. Refusing to run."; exit 90
  fi
done
...
')
[ -z "$TOPO_BAD" ] || { echo "FATAL: island-1 contains a non-NVLink GPU edge: $TOPO_BAD"; exit 93; }
...
[ "$USED" -lt 2000 ] || { echo "FATAL: island 1 is not idle (${USED} MiB in use)"; exit 92; }
echo "ISLAND_GUARD_PASS: 4 idle GPUs, no island-2 UUID visible"
...
manifest = {
    ...
    "metadata": {"name": job, "namespace": ns,
                 "labels": {"app": "lmdeploy-v100-next", "island": "1"}},
    "spec": {"backoffLimit": 0, "ttlSecondsAfterFinished": 86400,
        "template": {"metadata": {"labels": {"app": "lmdeploy-v100-next", "island": "1"}},
        "spec": {
            "restartPolicy": "Never",
            ...
            "containers": [{
                ...
                "env": [
                    {"name": "NVIDIA_VISIBLE_DEVICES", "value": island1},
                    {"name": "NVIDIA_DRIVER_CAPABILITIES", "value": "compute,utility"},
                    {"name": "ISLAND2_UUIDS", "value": island2},
                    ...
                    {"name": "TM_MTP_AMBIGUITY_MARGIN", "value": mtp_ambiguity_margin},
                    {"name": "TM_MTP_AMBIGUOUS_REPLAY", "value": mtp_ambiguous_replay},
            ...
            "volumes": [
                ...
                {"name": "src", "hostPath": {"path": "/localpool/lmdeploy-v100-island1/src/lmdeploy", "type": "Directory"}},
                {"name": "wheels", "hostPath": {"path": "/localpool/lmdeploy-v100-island1/wheels", "type": "DirectoryOrCreate"}},
                ...
                {"name": "results", "hostPath": {"path": "/localpool/lmdeploy-v100-island1/results", "type": "DirectoryOrCreate"}},
