    ...
    candidates_fn_(std::move(candidates))
{
    ...
    static const bool attention_qkv_torch_layout = [] {
    ...
    if ((torch_qkv_layout || attention_qkv_torch_layout) && fused_context_kv_projection
        && attention_indices_.size() == 5) {
...
Tensor DFlashPredictor::RunDraftLayers(Tensor hidden, int phase) const
{
    ...
    static const bool rank_ordered_allreduce = [] {
        const char* value = std::getenv("TM_DFLASH_RANK_ORDERED_ALLREDUCE");
        return value && value[0] == '1';
    }();
