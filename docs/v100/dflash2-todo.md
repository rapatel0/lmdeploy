...
- [ ] **Close the first downstream draft mismatch under exact runtime context.**
  ...
  - Exact normalized-Q/K replay made Q/K bit-identical and collapsed layer-0 core-attention RMS to `0.00116..0.00216` across TP4 while native prompt-cache K/V were already within RMS `0.00098..0.00172`. This rules out RoPE, cache layout, and core attention as the primary error and localizes it to Q/K projection-normalization numerics.
  - Matching SGLang's full-warp head-dim-128 QK reduction raised commit length from `2.283` to `2.333` and decode from `69.72` to `70.63` tok/s; audited 256-token identity passed.
  - TurboMind also inserted an extra FP16 round between normalization and weight multiplication. SGLang keeps the full product in FP32 and narrows once. Removing the extra round raised commit length from `2.311` to `2.371` and decode from `70.42` to `72.61` tok/s; audited identity passed. Both QK fixes are default-on with `TM_DFLASH_QK_NORM_WARP32=0` and `TM_DFLASH_QK_FULL_PRODUCT=0` as controls.
  - Preserving SGLang/PyTorch `[N,K]` physical qkv_proj storage did not help: commit changed from `2.340` to `2.333` and decode from `71.77` to `71.17` tok/s. The arm was removed. The remaining task is to reproduce SGLang's QKV GEMM result or identify the next projection-input arithmetic mismatch under exact replay.
  - Artifacts: `/results/20260830_205123-sglang-dflash-parity-e72d0ec7e484`, `/results/20260830_210147-dflash-actual-context-replay-141ca20ddf79`, `/results/20260831_043449-dflash-target-fp32-b38309313b24`, `/results/20260831_044110-dflash-target-fp32-followup-b38309313b24`, `/results/20260831_045913-dflash-qk-norm-warp32-f17cd6e07ca3`, and `/results/20260831_051123-dflash-qkv-transpose-3135b1166174`.
