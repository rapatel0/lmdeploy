...
- [ ] **Optimize target verification kernels in measured order.**
  ...
  - [ ] Test the separately owned `5120x62080` FP16 vocabulary head with row-major cuBLAS, transposed cuBLAS, and the unpacked native 8x64x64 SM70 kernel in one build. Require M=1/7/8 numeric parity, `postDecodeEmbedding` route proof, at least 10% isolated kernel gain, at least 1% isolated and combined normalized-cycle gain, and exact audited identity. The checkpoint tie is semantic: only `text_model.output` may be transposed; token embeddings remain a separate sharded allocation.
