...
- [ ] **Close the first downstream draft mismatch under exact runtime context.**
  ...
  - Matching SGLang's full-warp head-dim-128 QK reduction raised commit length from `2.283` to `2.333` and decode from `69.72` to `70.63` tok/s; audited 256-token identity passed. The full-warp route is default-on with `TM_DFLASH_QK_NORM_WARP32=0` as control.
