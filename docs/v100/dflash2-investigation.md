...
SGLang's head-dim-128 fused QK norm uses one full warp per head, four FP16 elements per lane, and an XOR reduction. TurboMind used half a warp and eight elements per lane. Matching SGLang's exact accumulation order passed audited 256-token identity, raised five-trial commit length from `2.283` to `2.333` (+2.2%), and raised decode from `69.72` to `70.63` tok/s (+1.3%). It is now default-on, with `TM_DFLASH_QK_NORM_WARP32=0` retaining the legacy reduction. A separate SGLang-layout `[N,K]` physical qkv_proj arm did not improve acceptance: commit changed from `2.340` to `2.333` and decode from `71.77` to `71.17` tok/s, so the layout experiment was removed.
