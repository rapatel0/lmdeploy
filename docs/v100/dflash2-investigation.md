...
A full target trajectory initially appeared to localize a large difference to target layer 0's dense FFN: the MLP-normalized input differed by only maximum absolute `0.0004883` and RMS `7.62e-5`, while the captured MLP-output row differed by maximum absolute `0.5195` to `0.6641`. Further tracing corrected that interpretation. SGLang's MLP hook observes the row-parallel down projection after its all-reduce, while TurboMind captured the local TP shard before `AllreduceResidualRMSnorm`; those rows are not the same boundary. The next shared post-allreduce residual differs by only maximum absolute `0.0001831`, RMS `1.96e-5` on every rank.

Exact replay made both the layer-0 MLP input and post-SwiGLU activation bit-identical. The local-versus-global MLP-output difference remained, as expected, while the production trajectory and normalized cycle were unchanged within variance. Shared output-residual RMS remains only `3.05e-5` after layer 1 and `5.50e-5` after layer 2. The target path is therefore closely aligned through the first three GDN layers; no target down-projection defect is established. A separate-SwiGLU arm still proved harmful—it accepted zero drafts and collapsed commit length to `1.0`—and was removed, but it is not a parity fix.

A Triton-attention rerun produced finite shared target boundaries through layer 5. Shared output-residual RMS grows only from `1.96e-5` after layer 0 to `1.01e-4` after layer 5. Direct audited prompt-feature capture at checkpoint target layers `[5,19,33,47,61]` shows gradual RMS drift of `0.000101`, `0.000446`, `0.001618`, `0.005997`, and `0.027449`. All 38 SGLang target trajectory source dtypes are FP16, proving that the inspected FP32-residual SM70 fusion is not active for this request. A direct TurboMind FP32-residual control confirmed the code audit: raw commit changed from `2.340` to `2.327`, effective commit from `2.311` to `2.327`, and decode regressed from `70.60` to `68.99` tok/s. Exact audited identity passed, but target-context parity was unchanged at RMS `2.2003`; the experiment was removed. Artifact: `/results/20260831_043449-dflash-target-fp32-b38309313b24` and follow-up `/results/20260831_044110-dflash-target-fp32-followup-b38309313b24`.

Replaying SGLang's exact five prompt residuals did not improve acceptance. Across five 1,536-token trials, commit length fell from `2.559` to `2.507`, decode fell from `77.80` to `75.94` tok/s, and the replay introduced 30 ambiguous steps that discarded 42 tokens. Prompt target-trajectory drift is therefore not a standalone acceptance fix.

The older `target.post_layer_residual` parity record differs from the pre-communication audited prompt residual by RMS `0.9232` and maximum absolute `39.5`, but row/layout tracing confirmed this is not an off-by-one or permutation: `project_target_hidden` consumes row 999 with the expected diagonal feature order. The difference is introduced by SGLang's capture communication path before ProjectContext. That post-communication tensor is the correct runtime input.

Replaying the exact post-communication SGLang ProjectContext input also regressed LMDeploy. Across five 1,536-token trials, commit/raw length fell from `2.800/2.831` to `2.495/2.520`, verification steps rose from `3,253` to `3,660`, and decode fell from `85.09` to `76.02` tok/s. Therefore neither the pre-communication prompt residual nor the actual post-communication SGLang context is a standalone acceptance fix. Context differences currently compensate for downstream draft/runtime differences; they cannot be changed in isolation.

Full downstream replay localizes the first material draft mismatch to layer-0 attention. With SGLang's exact post-communication context and exact initial normalized activation, block IDs, embeddings, initial norm, convolution projection, and convolution side 0 match. Layer-0 attention core output is the first large error (maximum absolute `17.125`, RMS `0.1462` in the reduced replay probe). Replaying SGLang's exact reduced attention output makes the remainder of layer 0 nearly identical through the MLP, ruling out grouped-convolution side 1 as the primary cause.

The next isolation closes the RoPE/cache/core alternatives. Replaying exact normalized Q and K while retaining TurboMind's prompt cache made normalized Q/K bit-identical and reduced layer-0 attention core-output RMS to `0.00116..0.00216` across TP4 ranks. Native prompt cache K/V were already close at RMS `0.00098..0.00172`. The primary error is therefore Q/K projection-normalization numerics before RoPE, amplified by attention, rather than cache layout or core-attention arithmetic.

SGLang's head-dim-128 fused QK norm uses one full warp per head, four FP16 elements per lane, and an XOR reduction. TurboMind used half a warp and eight elements per lane. Matching SGLang's accumulation order passed audited 256-token identity, raised five-trial commit length from `2.283` to `2.333` (+2.2%), and raised decode from `69.72` to `70.63` tok/s (+1.3%).

A second concrete QK arithmetic defect remained: SGLang retains `input * inv_rms * weight` in FP32 and narrows once, while TurboMind narrowed `input * inv_rms` to FP16 before multiplying the weight. Exact projected-Q/K replay proved the SGLang normalized output equals the single-round formula on real captured values. Removing the extra half round raised five-trial commit length from `2.311` to `2.371` (+2.6%) and decode from `70.42` to `72.61` tok/s (+3.1%); raw and effective commit also became equal in that matrix. Audited 256-token identity passed. Both fixes are default-on; `TM_DFLASH_QK_NORM_WARP32=0` and `TM_DFLASH_QK_FULL_PRODUCT=0` retain independent legacy controls.

A separate SGLang-layout `[N,K]` physical qkv_proj arm did not improve acceptance: commit changed from `2.340` to `2.333` and decode from `71.77` to `71.17` tok/s, so the layout experiment was removed.

Artifacts: `/results/20260829_032508-sglang-dflash-parity-b753831db680`, `/results/20260829_035037-dflash-context-replay-4fe9971622bc`, `/results/20260830_195423-dflash-target-mlp-replay-a93b7e210cdf`, `/results/20260830_200112-dflash-target-mlp-replay-10a7d78d05df`, `/results/20260830_202014-sglang-dflash-parity-f4a483fc5825`, `/results/20260830_203138-dflash-target-trajectory-8821fbf3867a`, `/results/20260830_203748-dflash-prompt-feature-replay-8821fbf3867a`, `/results/20260830_205123-sglang-dflash-parity-e72d0ec7e484`, `/results/20260830_210147-dflash-actual-context-replay-141ca20ddf79`, `/results/20260831_045913-dflash-qk-norm-warp32-f17cd6e07ca3`, and `/results/20260831_051123-dflash-qkv-transpose-3135b1166174`.
...
A rejected-verifier-K/V lifecycle intervention falsified suffix visibility as an acceptance cause. `TM_DFLASH_POISON_REJECTED_KV` overwrote every rejected target-attention K/V position across all 16 target attention layers while leaving the dedicated DFlash draft cache untouched. All four ranks proved the write. The poison arm exactly matched one complete repeated control across all 2,332 TP proposal records and the full acceptance summary at commit/raw length 2.631/2.652 over 577 steps; audited 128-token identity passed. Fresh-process controls can follow different known near-tie trajectories, so exact reproduction of either complete control is the robust null criterion. Artifact: `/results/20260830_002516-dflash-rejected-kv-poison-f0d92882a2c3`.

The separate post-rollback metadata hypothesis was confirmed. Aggregate draft-attention metadata remained based on the previous speculative extent even though live `cu_k_len` followed the accepted frontier. Rebuilding from `committed_sequence_length + block_size` corrected 2,188 traced metadata records, changed 8.59% of matched proposal blocks, and moved the first candidate difference to frontier 1165. Five-trial decode increased from 80.59 to 82.95 tok/s (+2.93%) as commit length increased from 2.631 to 2.716 (+3.23%), while normalized cycle cost changed only from 32.65 to 32.74 ms (+0.29%). A rebuilt-wheel confirmation strengthened the result to 80.62 versus 84.55 tok/s (+4.87%), commit length 2.631 versus 2.756 (+4.75%), and a 0.12% faster normalized cycle. One of three corrected 256-token identity arms passed exactly; the other two and all three stale controls split only at the established fresh-process near-tie position 145. No novel split occurred, and 1,124 live-span assertions passed. The corrected rebuild is default-on, with `TM_DFLASH_REBUILD_METADATA_AFTER_ROLLBACK=0` preserving the stale control. Artifacts: `/results/20260830_004745-dflash-metadata-rebuild-ebf7927919a5`, `/results/20260830_005648-dflash-metadata-rebuild-5faa9810904e`, and `/results/20260830_010502-dflash-metadata-identity-followup-2d2b33f76fbe`.
